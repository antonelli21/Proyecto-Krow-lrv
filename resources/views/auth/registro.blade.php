...
@if($errors->any())
{{-- Se muestran los errores de validación devueltos por el controlador --}}
@endif
@php
$carreras = [
    ...
    <form class="auth-form active" id="formCandidato" method="POST" action="{{ route('register.estudiante') }}" novalidate>
      ...
      <div class="form-group">
        ...
        <div class="input-wrap">
          ...
          {{-- old('email') mantiene el valor si falla la validación --}}
          <input type="email" id="c-email" name="email" placeholder="tu@email.com" autocomplete="email" value="{{ old('email') }}">
        </div>
        {{-- Error de validación del email (del servidor) --}}
        @error('email')
      ...
      <div class="form-group">
        ...
        <div class="input-wrap">
          ...
          {{-- Nombre 'password_confirmation' requerido por la regla 'confirmed' de Laravel --}}
    ...
    <form class="auth-form" id="formEmpresa" method="POST" action="{{ route('register.empresa') }}" novalidate>
      ...
      <div class="form-row" style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
        <div class="form-group">
          ...
          {{-- Error de validación del CUIT (incluye validación del dígito verificador) --}}
          @error('cuit')
          <span class="error-msg" style="display:flex;">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <circle cx="12" cy="12" r="10" />
...
<script>
  ...
  // 2. Cambio de Tabs (Candidato / Empresa)
  document.addEventListener('DOMContentLoaded', function() {
    const tabs = document.querySelectorAll('.role-tab');
    const formCand = document.getElementById('formCandidato');
    const formEmp = document.getElementById('formEmpresa');

    // Restaurar tab activo si hubo error de validación en uno u otro
    const hasEmpresaErrors = JSON.parse('{!! json_encode(old("cuit") || $errors->has("cuit") || $errors->has("nombre_empresa")) !!}');
    
    if (hasEmpresaErrors) {
        // Hubo un submit del form de empresa
